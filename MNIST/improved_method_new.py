...
# dict.value： multiple values in a list, 0 / 1+def init_multisection_coverage_value(model, multisection_num):
...
#     if test_img_num < 0 and test_img_num > total_img_num:
...
for i in range(test_img_num):
    ...
    #  Optimization 第一部分： 找到 c, c_topk = dnn.predict(Xs)
        ...
        def target_neurons_weights(model, model_layer_times, target_neuron_cover_num):
            ...
            for (layer_name, index), time in model_layer_times.items(): # get coverage so far
                if "conv" in layer_name or "softmax" in layer_name or 'fc' in layer_name:
        ...
        loss_neuron += target_neurons_weights(model, model_layer_times1, target_neuron_cover_num)
        final_loss = K.mean(K.mean(layer_output) + balance_lambda * K.sum(loss_neuron))
    ...
    # 1.定义 gradients backend函数: 求损失函数关于变量的导数，也就是网络的反向计算过程。
        ...
        grads_tensor_list.extend(loss_neuron) # extend: a list
