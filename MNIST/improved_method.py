...
sum_neuron_coverage = 0
sum_k_section_coverage = 0
sum_upper_corner_coverage = 0
...
for batch_no in range(batch_num):
    ...
    for i in range(seeds_num):
        ...
        img_name = os.path.join(img_dir, img_names_batches[batch_no][i]) # dir+name -> single img path, (name 即img_names_batches[i])
        if (i + 1) % 25 == 0:
        ...
        # get right labels
        img_name = img_names_batches[batch_no][i].split('.')[0] # extract img name without the path suffix(after the “.”）
        ...
        k_section_coverage, upper_corner_coverage = 0, 0
        if (i + 1) % 50 == 0:
    ...
    # end of a batch
    sum_neuron_coverage += neuron_coverage
    sum_k_section_coverage += k_section_coverage
    sum_upper_corner_coverage += upper_corner_coverage

print('\n--------------------Summary: %d batches, each size %d----------------------' % (batch_num, batch_size))
print("wrong prediction (cross decision boundary) %d/%d <=> %.3f\n" \
            % (wrong_predi, total_test_data_num, wrong_predi/total_test_data_num))

print("For each batch,")
print('avg neurons coverage %.3f, for %d neurons'
      % (sum_neuron_coverage/batch_num, total_neuron_num))

print('avg %d-section coverage: %.3f/%d <=> %.3f' % (multisection_num, \
                                    sum_k_section_coverage, batch_num, sum_k_section_coverage/batch_num))
print('avg upperCorner coverage: %.3f/%d <=> %.3f' % (sum_upper_corner_coverage, \
                                    batch_num, sum_upper_corner_coverage/batch_num))
# print('LowerCorner coverage: %d/%d <=> %.3f' % (len([v for v in lower_corner_times.values() if v > 0]), \
# k_section_neurons_num, len([v for v in lower_corner_times.values() if v > 0])/k_section_neurons_num))
try:
