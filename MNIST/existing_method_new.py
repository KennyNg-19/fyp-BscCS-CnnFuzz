...
neuron_to_cover_weight = float(sys.argv[5]) # Optimization 第二部分的λ 协调两部分: 越大，则以Neuron coverage为目标；越小，则以more adversrial为目标
...
save_dir = './gen_adversarial_existing_method_new/'
...
# ----------------------------------------------------------------
    ...
    # 完整的 Optimization 目标函数
        # obj = sum(c_topk) - c + λ · sum(neurons outputs)
        # to_be_optimized_loss = layer_output + neuron_to_cover_weight * K.sum(loss_neuron) # loss_neuron is a list

    # for adversarial image generation
        # final_loss = K.mean(to_be_optimized_loss) # 梯度的目标函数值
        ...
        def target_neurons_weights(model, model_layer_times, target_neuron_cover_num):
            ...
            for (layer_name, index), time in model_layer_times.items(): # get coverage so far
                if "conv" in layer_name or 'fc' in layer_name or "softmax" in layer_name:
        ...
        target_neurons_weights = target_neurons_weights(model, model_layer_times1, target_neuron_cover_num)
        final_loss = K.mean(neuron_to_cover_weight * K.sum(target_neurons_weights))

# ---------------------------------------------------grads = @obj/@xs--------------------------------------------------------------
    # 1.定义 gradients backend函数: 求损失函数关于变量的导数，也就是网络的反向计算过程。
        grads_tensor_list = [] # [loss_1, loss_2, loss_3, loss_4, loss_5]
        grads_tensor_list.extend(target_neurons_weights) # extend 加一个list的所有元素: extends the list by adding all items
    ...
    # 2.编译 gradient函数：将一个计算图（计算关系）编译为具体的函数。典型的使用场景是输出网络的中间层结果
        ...
        for iters in range(iteration_times): # 1 epoch, 最多一个 adversrial generation ≤ iteration_times 输入超参epoch
            ...
            loss_neuron_list = iterate([gen_img]) # a list
...
print('Unactivated: ', [neuron for neuron in model_layer_times2.keys() if model_layer_times2[neuron] == 0])
