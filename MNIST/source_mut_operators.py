...
#  np.random.permutation 打乱顺序
    ...
    # 2. LE (Label Error) 负相关: mutate_ratio越高，acc越低， 持续性影响很大 acc可能小于0.5
    ...
    def NP_mut(self, train_dataset, model, mutation_ratio, STD=0.1):
        ...
        permutation = np.random.permutation(number_of_train_data) # a list of INDEX!! Parameters: int or array_like If x is an integer, randomly permute np.arange(x).
        permutation_with_noise = permutation[:number_of_noise_perturbs] # a list of index!!! 取打乱顺序后的，0-ratio决定的部分
        ...
        for new_index in permutation_with_noise: # new_index 是chosen sample的index
